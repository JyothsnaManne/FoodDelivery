...
import { UserSignup } from '../actions/foodActions';
...
class SignUp extends Component {
    ...
    handlePincodeChange(event) {
        ...
        this.setState({
            ...
            pinLengthError: (this.user.userPincode.length !== 6) ? true : false
        })
        this.buttonFlag()
    }
    buttonFlag() {
        this.setState({
            disabledFlag: ((this.user.userName === "") || (this.user.userPassword === "") || (this.user.retypePassword !== this.user.userPassword) || (this.user.userPassword.length <= 6) || (this.user.retypePassword === "") || (this.user.userMobile === "") || (this.user.userMobile.length !== 10) || (isNaN(this.user.userMobile)) || (this.user.userAddress === "") || (this.user.userPincode === "")) ? true : false
        })
    ...
    handleSubmit(event) {
        ...
        this.props.UserSignup(this.user)
        this.user = new User()
        this.setState({
        ...
        return (
            <div className="container">
                ...
                <form onSubmit={(event) => this.handleSubmit(event)}>
                    ...
                    <div className="form-group">
                        <label>Pincode</label>
                        <input type="text" className="form-control" placeholder="Enter email" value={this.state.user.userPincode} onChange={(event) => this.handlePincodeChange(event)} />
                        {
                            (this.state.pincodeError) ?
                                <div className="alert alert-dismissible alert-danger">
                                    <button type="button" className="close" data-dismiss="alert">&times;</button>
                                    <strong>sorry!</strong> Email canot be Empty
                                </div> : (this.state.pinLengthError) ?
                                    <div className="alert alert-dismissible alert-danger">
                                        <button type="button" className="close" data-dismiss="alert">&times;</button>
                                        <strong>sorry!</strong> Length should be exactly 6
                                    </div> : null
                        }
                    </div>
